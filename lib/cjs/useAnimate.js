...
function useAnimate(target, keyframes, options) {
    const config = typeof options === 'object' ? { ...options } : { duration: options };
    const { immediate = true, commitStyles = false, persist = false, onReady, onError = (e) => console.error(e), ...animateOptions } = config;
    ...
    const [playState, setPlayState] = (0, react_1.useState)('idle');
    const [replaceState, setReplaceState] = (0, react_1.useState)('active');
    const [startTime, setStartTime] = (0, react_1.useState)(null);
    const [currentTime, setCurrentTime] = (0, react_1.useState)(null);
    const [timeline, setTimeline] = (0, react_1.useState)(null);
    const [playbackRate, setPlaybackRate] = (0, react_1.useState)(1);
    const updateAnimationState = (0, react_1.useCallback)(() => {
    ...
    const createAnimation = (0, react_1.useCallback)(() => {
        if (!isSupported || !target.current)
            return null;
    ...
    const reverse = (0, react_1.useCallback)(() => {
        try {
            if (!animateRef.current) {
                const animation = createAnimation();
                if (animation) {
