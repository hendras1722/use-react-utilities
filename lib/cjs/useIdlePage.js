...
exports.useIdlePage = useIdlePage;
const react_1 = require("react");
function useIdlePage(options = {}) {
    ...
    const [isIdle, setIsIdle] = (0, react_1.useState)(false); // Current tab idle state
    const [isSystemIdle, setIsSystemIdle] = (0, react_1.useState)(true); // All tabs idle state
    const tabId = (0, react_1.useRef)(Math.random().toString(36).slice(2, 15)).current; // Generate tab ID only once
    const [lastActivity, setLastActivity] = (0, react_1.useState)(Date.now());
    // Timers (using useRef to persist across renders)
    const idleCheckTimer = (0, react_1.useRef)(undefined);
    // Get data from storage (memoized with useCallback)
    const getStorageData = (0, react_1.useCallback)(() => {
        try {
            const storedData = localStorage.getItem(config.storageKey);
            return storedData ? JSON.parse(storedData) : {}; // Type assertion here
        }
        catch {
            return {}; // Type assertion here
        }
    }, [config.storageKey]);
    // Update storage with new data (memoized with useCallback)
    const updateStorage = (0, react_1.useCallback)((data) => {
        // Explicit type for data
        localStorage.setItem(config.storageKey, JSON.stringify(data));
    }, [config.storageKey]);
    // Check if all tabs are idle (memoized with useCallback)
    const checkGlobalIdleState = (0, react_1.useCallback)(() => {
        ...
        for (const tab of Object.values(cleanedTabs)) {
            if (tab && tab.idle === false)
                allIdle = false; //  Type check and guard
            hasActiveTabs = true;
        }
    ...
    const updateTabStatus = (0, react_1.useCallback)(() => {
        const allTabs = getStorageData();
        const newTabStatus = {
            lastUpdate: Date.now(),
            idle: isIdle,
    ...
    const removeTabFromStorage = (0, react_1.useCallback)(() => {
        const allTabs = getStorageData();
    ...
    const handleUserActivity = (0, react_1.useCallback)(() => {
    ...
    const handleVisibilityChange = (0, react_1.useCallback)(() => {
        updateTabStatus();
    }, [updateTabStatus]);
    ...
    const startIdleDetection = (0, react_1.useCallback)(() => {
    ...
    const stopIdleDetection = (0, react_1.useCallback)(() => {
