...
export function useMouse(options = {}) {
    ...
    useEffect(() => {
        ...
        // Mouse events
        const mouseCleanup = useEventListener(target, ['mousemove', 'dragover'], mouseHandlerWrapper, listenerOptions) ?? (() => { });
        cleanupFunctions.push(mouseCleanup);
        // Touch events
        if (touch && type !== 'movement') {
            const touchCleanup = useEventListener(target, ['touchstart', 'touchmove'], touchHandlerWrapper, listenerOptions) ?? (() => { });
            cleanupFunctions.push(touchCleanup);
            if (resetOnTouchEnds) {
                const touchEndCleanup = useEventListener(target, 'touchend', resetPosition, listenerOptions) ?? (() => { });
                cleanupFunctions.push(touchEndCleanup);
            }
        }
        // Scroll events
        if (scroll && type === 'page') {
            const scrollCleanup = useEventListener(window, 'scroll', scrollHandlerWrapper, listenerOptions) ?? (() => { });
