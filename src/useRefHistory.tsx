...
export default function useRefHistory<Raw, Serialized = Raw>(
  initialValue: Raw,
  ...
  const {
    ...
    capacity,
    clone = deep,
    dump = (v: Raw) => v as unknown as Serialized,
    parse = (v: Serialized) => v as unknown as Raw,
  } = options

  const [value, setValueState] = useState<Raw>(initialValue)
  const [isTracking, setIsTracking] = useState(true)
  const [lastUpdated, setLastUpdated] = useState(Date.now())

  const historyRef = useRef<HistoryRecord<Serialized>[]>([])
  const undoStack = useRef<HistoryRecord<Serialized>[]>([])
  const skipNextCommit = useRef(false)
  const isInitialized = useRef(true)

  const cloneValue = useCallback(
    (val: Raw): Raw => {
      if (!clone) return val
      if (typeof clone === 'function') return (clone as CloneFn<Raw>)(val)
  ...
  const areValuesEqual = useCallback(
    (a: Serialized, b: Serialized): boolean => {
  ...
  const setValue = useCallback((newValue: Raw | ((prev: Raw) => Raw)) => {
  ...
  const undo = useCallback(() => {
    ...
    if (!previous || historyRef.current.length === 1) return
